...
function appModal(opts){return new Promise(resolve=>{appModalTitle.textContent=opts.title;appModalMessage.textContent=opts.message;appModalCancel.classList.toggle("hidden",!opts.showCancel);appModalOverlay.classList.remove("hidden");const done=result=>{appModalOverlay.classList.add("hidden");appModalOk.onclick=null;appModalCancel.onclick=null;resolve(result)};appModalOk.onclick=()=>done(true);appModalCancel.onclick=()=>done(false)})}
function appAlert(message,title="Aviso"){return appModal({title,message,showCancel:false})}
function appConfirm(message,title="Confirmar"){return appModal({title,message,showCancel:true})}
